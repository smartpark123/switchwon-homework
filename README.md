...
- 현재 구현은 과제 요구사항 범위(인증/세션 제외, 주문 환산/이력 저장)에 집중했습니다.
- 실서비스 확장 시 사용자별 보유 잔고 도메인을 추가하고, 매도 주문 시 보유 수량 검증이 필요합니다.
- 외부 환율 API 장애 시 자동 fallback/retry/backoff 정책을 강화할 수 있습니다.
- 다중 인스턴스 환경에서는 스케줄러 중복 실행 방지를 위한 분산 락 적용을 고려할 수 있습니다.
- 최신 환율 조회 API는 수집 주기(1분)에 맞춘 단기 캐시(TTL 30~60초) 적용으로 조회 성능을 개선할 수 있습니다.